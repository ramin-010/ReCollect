...
const SubtaskWidgetComponent: React.FC<NodeViewProps> = ({ node, updateAttributes, deleteNode, editor }) => {
  ...
  return (
    <NodeViewWrapper className="my-4 mr-6" contentEditable={false} as="div">
      <div 
        className="border border-white/10 rounded-lg bg-[#1e1e1e]/50 overflow-hidden"
        onMouseDown={(e) => {
      ...
      >
        <div className="px-4 py-2 text-xs font-medium text-white/40 border-b border-white/5 bg-[#1e1e1e] flex justify-between items-center group/header">
          <span>Sub-issues ({subtasks.filter(s => s.isCompleted).length}/{subtasks.length})</span>
          <button onClick={deleteNode} className="opacity-0 group-hover/header:opacity-100 text-white/20 hover:text-white/40 transition-all"><X className="w-3 h-3" /></button>
        </div>
          ...
          {subtasks.map((st, idx) => (
            <div key={st.id} className="flex items-center gap-3 px-3 py-2 group/item">
              ...
              <input
                ...
                className={cn(
                  "flex-1 bg-transparent text-sm placeholder:text-white/30 focus:outline-none min-w-[50px]", 
              ...
              <button 
                ...
                className="opacity-0 group-hover/item:opacity-100 p-1 text-white/20 hover:text-white/40 transition-all"
              >
                <X className="w-3 h-3" />
              </button>
            </div>
