  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-white/40">Personalize your workspace appearance and details.</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl border border-white/5 bg-white/[0.01]">
          <div>
            <h3 className="text-sm font-medium text-white mb-1">Overview Dashboard Visibility</h3>
          ...
          <div className="shrink-0 flex items-center gap-3">
            {isUpdating && <Loader2 className="w-4 h-4 text-white/40 animate-spin" />}
            <button
              disabled={!isAdmin || isUpdating}
              onClick={() => handleToggleOverview(!membersCanViewOverview)}
              className={cn(
                "relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#1E1E1E] disabled:cursor-not-allowed disabled:opacity-50",
                membersCanViewOverview ? "bg-indigo-500" : "bg-white/10"
              )}
            >
